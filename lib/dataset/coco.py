...
class COCODataset(JointsDataset):
    ...
    '''
    def __init__(self, cfg, root, image_set, is_train, transform=None, isKFold=False, imIds=[]):
        ...
        if not isKFold:
            ...
            self.image_set_index = imIds
        self.num_images = len(self.image_set_index)
        print(self.num_images)
        logger.info('=> num_images: {}'.format(self.num_images))

        self.num_joints = 15
        self.flip_pairs = [[1, 3], [2, 3], [3, 15], [4, 5],
                           [4, 15], [6, 7], [6, 15], [8, 9], [8, 11], [9, 10], [11, 12], [12, 13], [8, 14], [14, 15]]
    ...
    def _load_coco_person_detection_results(self):
        ...
        for n_img in range(0, len(all_boxes)):
            ...
            if det_res['category_id'] != 1:
                continue
            img_name = self.image_path_from_index(int(det_res['image_id']))
            box = det_res['bbox']
            score = det_res['score']
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path,
        ...
        if 'test' not in self.image_set:
            ...
            return name_value, name_value['AP'], results
        else:
            return {'Null': 0}, 0, results
